...
You could also achieve this with a \emph{cast} \texttt{2/float(3)} or multiplication \texttt{(1.0*2)/3}.
 ...
 \item Signed vs Unsigned --- integer types can be restricted to store only postive values\footnote{and therefore store higher maxiumum values using the same number of bits because the sign bit can be used for value storage.}
...
The folowing are also operators in C but we won't talk about them here.
